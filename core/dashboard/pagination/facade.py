...
def paginate_list(tip: str, request: HttpRequest) -> Optional[PageResult]:
    ...
    if spec.engine == "keyset":
        raise NotImplementedError(
            f"Keyset-движок пагинации для tip={tip!r} ещё не реализован. "
            "Будет добавлен в core/dashboard/pagination/keyset_engine.py "
            "(см. план: этап подключения Keyset Engine к result/question/др.)."
        )
