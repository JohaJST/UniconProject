...
class RefreshRace(Exception):
    """
    Не атака, а безобидная гонка параллельных запросов с одним и тем же
    refresh-токеном (несколько вкладок / автосейв + навигация, пришедшие
    в middleware одновременно с истечением access-токена).

    Означает, что:
      - другой параллельный запрос прямо сейчас держит лок на ротацию
        этой же refresh-семьи, либо
      - другой параллельный запрос уже успешно ротировал семью долями
        секунды раньше нас.

    В обоих случаях сессия пользователя остаётся полностью валидной —
    новые токены выпустил параллельный запрос. Разлогинивать нельзя.
    """
    def __init__(self, payload: dict):
        super().__init__("Refresh race detected, session remains valid")
